...
(function($) {
  ...
  ATL_JQ.IssueDialog = function(options) {
    ...
    if (!options.baseUrl) {
      options.baseUrl = "https://ccg.garvan.org.au/jira"
    }
    this.options = options;
    this.frameUrl = options.baseUrl + "/rest/collectors/1.0/template/form/"
        + this.options.collectorId + "?os_authType=none";
    ...
    if (this.options.triggerPosition === "CUSTOM") {
      ...
      $(function() {
        try {
          ...
          if (window.ATL_JQ_PAGE_PROPS
              && ((window.ATL_JQ_PAGE_PROPS.triggerFunction) || (window.ATL_JQ_PAGE_PROPS["5ee9f010"] && window.ATL_JQ_PAGE_PROPS["5ee9f010"].triggerFunction))) {
            newTriggerFunction = window.ATL_JQ_PAGE_PROPS.triggerFunction
                || window.ATL_JQ_PAGE_PROPS["5ee9f010"].triggerFunction
          } else {
      ...
      if ($.isFunction(this.options.triggerPosition)) {
        ...
        if (this.options.triggerPosition && this.options.triggerText) {
          var triggerClass = "atlwdg-trigger atlwdg-"
              + this.options.triggerPosition;
          var $trigger = $("<a href='#' id='atlwdg-trigger'/>").addClass(
              triggerClass).text(this.options.triggerText);
          $body.append($trigger);
          $trigger.click(showDialog)
        }
      }
    }
    var $iframeContainer = $("<div id='atlwdg-container'/>").addClass(
        "atlwdg-popup atlwdg-box-shadow atlwdg-hidden");
    var $blanket = $("<div id='atlwdg-blanket' class='atlwdg-blanket'/>")
        .hide();
      ...
      if (this.options.triggerPosition === "TOP") {
        ...
        if (this.options.triggerPosition === "RIGHT") {
          triggerAdjuster = function(e) {
            ...
            $trigger.css("top", ($(window).height() / 2 - $trigger.outerWidth()
                / 2 + $(window).scrollTop())
                + "px");
            if (!ATL_JQ.isQuirksMode() && browser.version === "8.0") {
              $trigger.css("right", -($trigger.outerHeight() - $trigger
                  .outerWidth())
                  + "px")
            }
          }
          ...
          if (this.options.triggerPosition === "SUBTLE") {
            ...
            triggerAdjuster = function(e) {
              ...
              $trigger
                  .css("top",
                      ($window.scrollTop() + $window.height() - outerHeight)
                          + "px")
            }
          }
        }
  ...
  ATL_JQ.IssueDialog.prototype = {
    ...
    show : function() {
      var that = this, $iframeContainerElem = $("#atlwdg-container"), $body = $("body"), $iframeElem = $('<iframe id="atlwdg-frame" scrolling="no" frameborder="0" src="'
          + this.frameUrl + '"></iframe>'), $loadingImage = $('<img class="atlwdg-loading" style="display:none;" src="'
      ...
      if (this.options.collectFeedback) {
        ...
        for ( var prop in feedback) {
          if (feedback.hasOwnProperty(prop) && feedback[prop] !== undefined
              && feedback[prop] !== "" && typeof feedback[prop] === "string") {
            feedbackString += "*" + prop + "*: " + feedback[prop] + "\n"
          }
        }
      }
      var fieldValues = {};
      if (this.options.fieldValues
          && !$.isEmptyObject(this.options.fieldValues)) {
      ...
      $iframeElem.load(function() {
        ...
        $iframeElem[0].contentWindow.postMessage(JSON.stringify(message),
            that.options.baseUrl);
        $(window).bind(
            "message",
            function(e) {
              if (e.originalEvent.data
                  && e.originalEvent.data === "cancelFeedbackDialog") {
                that.hide()
              }
            })
      });
      ...
      dummyElement.href = "https://ccg.garvan.org.au/jira";
  ...
  var filterStrings = function(obj, recursingIn) {
    for ( var key in obj) {
      ...
      if (typeof value !== "string") {
        var paramName = recursingIn === undefined ? key : recursingIn + ":"
            + key;
        console.log("bootstrap.js:filterStrings ignoring key for value '"
            + paramName + "'; typeof must be string");
        delete obj[key]
      }
    }
    return obj
  ...
  ATL_JQ(function() {
    if ("5ee9f010" == "" && typeof ATL_JQ_CONFIGS == "undefined") {
      return
    }
    var showTrigger = function(triggerConfig, collectorId) {
      ...
      exports.dialog = new ATL_JQ.IssueDialog({
    ...
    if (typeof ATL_JQ_CONFIGS != "undefined") {
      ...
      var baseUrl = "https://ccg.garvan.org.au/jira";
      $
          .ajax({
            url : baseUrl
                + "/rest/collectors/1.0/configuration/trigger/5ee9f010?os_authType=none",
            dataType : "jsonp",
            crossDomain : true,
            jsonpCallback : "trigger_5ee9f010",
            cache : true,
            success : function(result) {
              showTrigger(result, "5ee9f010")
            }
          })
    }
  });
