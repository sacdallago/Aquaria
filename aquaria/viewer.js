...
//var JNLP_TOOLKIT_TEMPLATE = fs.readFileSync(__dirname + '/../views/applicationToolkitViewerJNLP.ejs', 'utf8');
...
//  console.log('key : ' + key + ', keyString: ' + args[keyString]);
};
...
var setupAttributes = function (development, attributes) {
  ...
  if (development) {
    ...
    attributes.host = config.get('host');
  ...
  attributes.port = config.get('port');
...
//	var md5 = common_utils.createMD5(instanceId);
...
//    	  console.log("Creating app file: " + args.pdb_id + ", at location: " + jnlpApplcationFilePath + ", args: " + JSON.stringify(args) );
...
//    	  console.log("Creating toolkit file: " + args.pdb_id + ", at location: " + jnlpApplcationFilePath);
//
//    	  var newApplcationJnlpContent = ejs.render(JNLP_TOOLKIT_TEMPLATE, args);
//    	  fs.writeFile(jnlpApplcationFilePath, newApplcationJnlpContent, function (err) {
//    		  if (err) throw err;
	    	  callback(jnlpWebLocation);
//    	  });
